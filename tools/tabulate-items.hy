(require [kodhy.macros [afind-or λ ecase]])

(import
  sys
  re
  subprocess
  [kodhy.util [T F keyword->str]]
...
(defn show-items [title predicate]
  ...
  (print "--------------------------------------------------")
  (for [item (sorted items :key (λ (, (- (it.generation-weight 0)) (- (or it.level-hi 9999)) it.price it.tid)))]
