...
Dependencies include Hy_ (0.10.1, and no later), Kodhy_, and Heidegger_. Currently, curses is also a requirement. (All dependencies not already included with Python are in pure Python or Hy.) You can download the game and its dependencies as a self-contained bundle at http://arfer.net/downloads/roguetv-posix.tar.gz . The bundle has been tested on Linux and Mac OS X and should also work on other Unix-likes. I will probably provide a proper Windows port eventually.
